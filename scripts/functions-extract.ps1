...
DESCRIPTION:
    Extracts all FUN_ functions from Ghidra using REST API calls for the specified DLL
    and formats them as "[ ] FUN_035b14f0 @ 035b14f0" for todo tracking.
    ...
    do {
        ...
        if ($response.StatusCode -eq 200) {
            ...
            foreach ($line in $lines) {
                $line = $line.Trim()
                # Match pattern: FUN_hexaddress at hexaddress
                if ($line -match '^(FUN_[0-9a-fA-F]+)\s+at\s+([0-9a-fA-F]+)') {
                    $funcName = $matches[1]
                    $address = $matches[2]
            ...
            if ($batchFunctions.Count -gt 0) {
                ...
                Write-Host "Found $($batchFunctions.Count) FUN_ functions in this batch" -ForegroundColor Green
            } else {
                Write-Host "No FUN_ functions found in this batch" -ForegroundColor Yellow
                break
            }
