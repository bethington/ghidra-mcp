#!/usr/bin/env python3
"""
REST Endpoint Feature Test

This script tests specific functionality of key REST endpoints with real data.
"""
...
def test_rest_endpoints():
    ...
    print("🚀 Testing GhidraMCP REST Endpoint Features")
    print("=" * 60)
    
    # Test 1: Get current program info
    print("\n📍 Test 1: Current Program Information")
    try:
        addr_resp = requests.get(f"{base_url}/get_current_address", timeout=5)
        func_resp = requests.get(f"{base_url}/get_current_function", timeout=5)
    ...
    try:
        ...
        decompile_resp = requests.get(f"{base_url}/decompile_function", timeout=15)
        ...
        disasm_resp = requests.get(f"{base_url}/disassemble_function", timeout=15)
    ...
    try:
        ...
        dt_resp = requests.get(f"{base_url}/list_data_types", timeout=10)
