...
- `isAutoGeneratedGlobalName` now recognizes Ghidra `s_<text>_<addr>` and
  `u_<text>_<addr>` string labels, eliminating false-positive
  `name_missing_g_prefix` audit hits on every defined string
- `autoFixFieldPrefix` preserves and derives `pp`/`pfn`/`a` prefixes for
  pointer-to-pointer, function-pointer, and array struct fields instead of
  degrading correct caller-supplied names to bare `p`
- `@Param defaultValue` is honored for boxed `Integer`/`Boolean` parameters
  in both QUERY and BODY sources; the `/mcp/schema` advertisement and the
  runtime binding now agree
- `set_function_prototype` matches the calling convention only in the head
  (before the parameter list), so a `__cdecl` callback parameter no longer
  hijacks the convention of a `__stdcall` function
- `create_function_signature` keeps all supplied parameters instead of only
  the last one (the loop called `setArguments` per iteration, replacing the
  list each time)
- `create_typedef` preserves multi-level pointer depth by delegating to
  `ServiceUtils.resolveDataType` (`int **` no longer collapses to `int *`).
  Note: an unresolvable pointer base now falls back to `void *` (the shared
  `resolveDataType` contract used by struct/union/field creation) instead of
  returning a "Base type not found" error
- `create_data_type_category` wraps the DataTypeManager mutation in
  `threadingStrategy.executeWrite` so it opens a transaction in GUI mode
  (previously threw `No open transaction` and always failed)
- `analyze_for_documentation` forwards `programName` to its nested
  completeness call so `?program=` targets the requested program rather than
  the active one
- Call-graph endpoints (`get_function_call_graph`, `get_full_call_graph`,
  `analyze_call_graph`) key on namespace-qualified `name@address` instead of
  bare function name, so same-named functions in different namespaces,
  overloads, or overlay spaces are no longer collapsed into one node;
  `json_edges` dedupes on callee address; text-format output now shows
  `name@addr` nodes
