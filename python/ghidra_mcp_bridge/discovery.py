...
from .transport import get_socket_dir_candidates, tcp_request, uds_request
from .validation import is_pid_alive
...
def discover_active_tcp_instance() -> dict | None:
    ...
    if connection.transport_mode() != "tcp" or not active_tcp:
        return None
    ...
    try:
        text, status = tcp_request(active_tcp, "GET", "/mcp/instance_info", timeout=5)
        if status == 200:
            info.update(_unwrap_response_data(text))
            return info
    except Exception as e:
        logger.debug(f"Could not query TCP instance info for {active_tcp}: {e}")

    try:
        text, status = tcp_request(active_tcp, "GET", "/list_open_programs", timeout=5)
        if status == 200:
        ...
        logger.debug(
            f"Could not query open programs for active TCP instance {active_tcp}: {e}"
        )
