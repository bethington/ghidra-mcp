...
| `scripts/fid/build-vc6-fiddb.ps1`, `scripts/fid/CountFidMatches.java` | Manual A/B against a binary: import once with stock FID, once with `-preScript AttachFidDatabase.java`, diff `CountFidMatches` output. **Building a VC6 FidDb was measured worthless** (D2Common 175→176, D2Client 216→216): Ghidra already ships `Ghidra/Features/FunctionID/data/vsOlder_x86.fidbf` plus vs2012–vs2019. They are `.fidbf` (PACKED) — a search for `*.fidb` finds nothing and invents a coverage gap that isn't there. Do NOT build a UCRT database expecting a different answer; `vs2019_x86.fidbf` covers the modern mod DLLs. The low corpus-wide match rate (4,325/70,146) is a limit of hash-exact matching against inlined/re-optimised CRT, not a missing library. Also: `analyzeHeadless` EXITS 0 when a script throws — scrape the log for `SCRIPT ERROR` and verify the artifact, or you will 'succeed' into an empty 671-byte .fidb. |
