...
| `fun-doc/doc_lint.py` — documentation *correctness* linter | `tests/performance/test_doc_lint.py` (offline) + a live sweep (`python doc_lint.py --folder /Mods/PD2-S12`). Complements `analyze_function_completeness`, which only measures whether documentation is present and well-formed — a function can score COMPLETE_90 while carrying `LIB_CRT` **and** a gameplay-domain name (measured: `DATATBLS_DecimalStringToDouble`, `CLIENT_IsAllZeros`). Findings are tiered: T1 = contradicts a curated `LIB_*` tag, T2 = heuristic, review before acting. Do NOT drop `RUNTIME_PREFIXES` or `EH_ONLY_CALLEES`: pure corpus calibration flagged `CRT_Init` as a defect (the conservative detector recognised only 10 of 79 `CRT_` functions, so `CRT_` read 87% "non-library"), and `_CxxThrowException` as library evidence misfiled hand-written `PD2_AllocItemExtraData` as CRT. Those two guards cut a 43-finding run to 14 real ones. |
