...
| `src/main/java/com/xebyte/core/LanguageVariants.java` + the `variant` gate (`ServiceUtils.variantGate` / `variantRejectionData`) | Offline (Java) `LanguageVariantsTest` for the decision table (it is deliberately Ghidra-free so the table runs without an install); after deploy `tests/integration/test_readonly_endpoints.py::TestDecompileVariantGate`, which auto-skips against a server built before the gate rather than reporting its absence as failures. **Ghidra's loader GUESSES the variant column and a wrong guess is undetectable in the output**: `PowerPC:BE:32:default` and `PowerPC:BE:64:VLE-32addr` (VLE) decode the same bytes into different instruction streams, so a VLE image opened as classic PowerPC decompiles into C that is syntactically perfect, entirely fictional, and reported as a success. The bucket is `(processor, endian, size)` — **`size` is the address size, not the id's middle field**, so `PowerPC:BE:64:VLE-32addr` (size 32) lands in the same bucket as `PowerPC:BE:32:default`, which is exactly the VLE-vs-classic choice this exists for; widening it to "same processor" would offer a 32-bit program a menu of 64-bit answers. **All FOUR endpoints that return decompiler-produced C consult the same check** (`/decompile_function` incl. its `functions=` bulk mode, `/force_decompile`, `/analyze_function_complete`, `/analyze_for_documentation`) — gating only the two decompile tools would have left the identical pseudocode reachable one tool over, the same shape as the eviction guard that a worker stepped around via `apply_data_type`. The composites **withhold `decompiled_code` and return the rest** (`decompiled_code_withheld` carries the refusal) rather than failing wholesale; endpoints that decompile only to derive a score or a field-usage map (`/analyze_function_completeness`, `/analyze_struct_field_usage`) are NOT gated, because they hand back no pseudocode. A mismatch is a refusal, never a warning: the decompiler decodes with the language the program was imported under and cannot be asked for another, so answering would return one dialect's C under a label the caller chose. Internal Java callers bypass the gate on purpose (`FunctionService.decompileAt`) — it belongs on the MCP boundary, where the question is asked. Adding a fifth C-returning endpoint without routing it through `variantGate` re-opens the hole. |
...
- **Decompiler output is only as correct as the language variant** — Ghidra's loader GUESSES the variant column, and `PowerPC:BE:32:default` vs `PowerPC:BE:64:VLE-32addr` (VLE) decode the same bytes into different instructions, so a wrong guess yields pseudocode that is syntactically perfect and fictional with nothing in the response saying so. `decompile_function` / `force_decompile` therefore REQUIRE `variant=` on any processor offering more than one at the program's endian/size. Measured against 12.1.2's `.ldefs`: **30 of 83 processor/endian/size buckets offer a choice, covering 124 of 177 non-deprecated languages** — x86 at BOTH 32-bit (`default` vs `System Management Mode`) and 64-bit (`default` vs `compat32`), ARM/LE/32 (ten), PowerPC/BE/32 (eleven) — so expect this on essentially every mainstream target, not just embedded ones. `get_language_metadata` reports `variant_required` + `available_variants`. The server structurally cannot re-decode under another variant: fixing a wrong one means `import_file(language=...)` or Ghidra's Language > Set Language.
