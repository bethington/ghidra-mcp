...
public class DataTypeService {
    ...
    public Response auditGlobal(
        ...
        if (autoGen) {
            ...
            if (!q.ok) issues.add("name_" + q.issue);
        }

        // Type
        if (data == null || hasUndefinedType) issues.add("untyped");

        // Bytes formatting
        if (data != null) {
            if (lengthMismatch) issues.add("unformatted_bytes_length_mismatch");
            // String detection: char/byte type at an address with a contiguous
            // printable run ending in null suggests a string type was missed.
            if (!typeName.isEmpty() && (typeName.equals("char") || typeName.equals("byte"))) {
                if (looksLikeUnappliedString(program, addr)) {
        ...
        Map<String, Object> result = JsonHelper.mapOf(
                ...
                "issues", issues,
                "fully_documented", issues.isEmpty()
        );
        return Response.ok(result);
    }

    private boolean looksLikeUnappliedString(Program program, Address addr) {
        // Heuristic: ≥4 printable ASCII bytes followed by a null terminator
        // within the next 64 bytes. Cheap; no false positives on numeric data.
        try {
