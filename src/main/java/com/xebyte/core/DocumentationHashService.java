    ...
    public Response getBulkFunctionHashes(
        ...
        try {
            ...
            for (Function func : funcMgr.getFunctions(true)) {
                ...
                funcItem.put("hash", hash);
                funcItem.put("instruction_count", instructionCount);
                funcItem.put("has_custom_name", isDocumented);
                functionsList.add(funcItem);
                ...
                if (func != null) {
                    ...
                    if (!seenFunctions.contains(funcName)) {
                        ...
                        if (ServiceUtils.isAutoGeneratedName(funcName)) {
                            undocCount++;
                            Map<String, Object> undocItem = new LinkedHashMap<>();
                            undocItem.put("name", funcName);
                            undocItem.putAll(ServiceUtils.addressToJson(func.getEntryPoint(), program));
                            undocItem.put("ref_address", fromAddr.toString(false));
                            if (ServiceUtils.getPhysicalSpaceCount(program) > 1) {
                                undocItem.put("ref_address_full", fromAddr.toString());
                                undocItem.put("ref_address_space", fromAddr.getAddressSpace().getName());
                            }
                            undocItem.put("ref_type", ref.getReferenceType().getName());
                            undocumentedList.add(undocItem);
                        } else {
                            docCount++;
                        }
                    }
                    ...
                    if (value instanceof String) {
                        ...
                        if (strValue.toLowerCase().contains(pattern.toLowerCase())) {
                                ...
                                if (func != null) {
                                    ...
                                    if (ServiceUtils.isAutoGeneratedName(funcName)) {
                                        ...
                                        if (!undocFuncKeys.contains(key)) {
                                            ...
                                            Map<String, Object> undocFunc = new LinkedHashMap<>();
                                            undocFunc.put("name", funcName);
                                            undocFunc.putAll(ServiceUtils.addressToJson(func.getEntryPoint(), program));
                                            undocFuncsList.add(undocFunc);
                                        }
                                    } else {
                            ...
                            if (!undocFuncsList.isEmpty() || !docFuncsList.isEmpty()) {
                                ...
                                Map<String, Object> anchorItem = new LinkedHashMap<>();
                                anchorItem.put("string", strValue);
                                anchorItem.putAll(ServiceUtils.addressToJson(strAddr, program));
                                anchorItem.put("undocumented", undocFuncsList);
                                anchorItem.put("documented", docFuncsList);
                                anchorItem.put("undocumented_count", undocFuncsList.size());
                                anchorItem.put("documented_count", docFuncsList.size());
