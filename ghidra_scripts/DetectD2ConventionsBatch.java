// Batch detect D2 custom calling conventions in loaded binary
// @author GhidraMCP
// @category Analysis
// @keybinding 
// @menupath Analysis.D2.Detect Calling Conventions
// @toolbar 
...
public class DetectD2ConventionsBatch extends GhidraScript {
    ...
    public void run() throws Exception {
        ...
        println("========================================");
        println("D2 CALLING CONVENTION BATCH DETECTION");
        println("========================================");
        ...
        for (Function func : funcManager.getFunctions(true)) {
            ...
            if (progress % 100 == 0) {
                ...
                println(String.format("[%d/%d] Analyzed: %d | Standard: %d | Unknown: %d | D2: %d | ETA: %d sec",
                    progress, totalFunctions, totalAnalyzed, standardCount, unknownCount, 
                    getTotalD2Detections(), eta));
            }
        ...
        for (String conv : D2_CONVENTIONS) {
            ...
            if (!results.isEmpty()) {
                ...
                for (int i = 0; i < Math.min(20, results.size()); i++) {
                    ...
                    println(String.format("  - %s @ %s (%.0f%% confidence)",
                        r.name, r.address, r.confidence * 100));
                }
