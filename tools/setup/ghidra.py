...
# The benchmark fixture lives in this repository and is generated by
# tests/fixtures/benchmark/make_fixture.py -- see that file for why it is
# emitted rather than compiled. Both binaries are committed, so a checkout is
# enough; the generator only has to run when someone changes it.
BENCHMARK_FIXTURE_DIR = Path("tests") / "fixtures" / "benchmark"
BENCHMARK_FIXTURE_GENERATOR = BENCHMARK_FIXTURE_DIR / "make_fixture.py"
DEFAULT_BENCHMARK_DLL = BENCHMARK_FIXTURE_DIR / "Benchmark.dll"
DEFAULT_BENCHMARK_DEBUG_EXE = BENCHMARK_FIXTURE_DIR / "BenchmarkDebug.exe"
...
BENCHMARK_DEPLOY_TEST_MODES = {
    "benchmark-read",
    "benchmark-write",
    # negative-contract belongs here: run_deploy_tests resets the fixture for
    # it like every other entry. It was missing, so `--test negative-contract`
    # imported two binaries without first enabling the prompt policy that keeps
    # Ghidra's import dialogs from blocking the HTTP thread, and skipped the
    # restored-tool cleanup afterwards.
    "negative-contract",
    "release",
    "debugger-live",
    "multi-program",
...
def reset_benchmark_fixture(repo_root: Path, mcp_url: str) -> None:
    ...
    if not benchmark_dll.is_file() or not benchmark_debug_exe.is_file():
        generator = repo_root / BENCHMARK_FIXTURE_GENERATOR
        if not generator.is_file():
            # Both the binaries and their generator are committed, so reaching
            # here means the checkout is incomplete -- not that a build failed.
            # Say which, because the previous version of this path could only
            # report a CalledProcessError from a script that did not exist.
            raise RuntimeError(
                f"Benchmark fixture generator is missing: {generator}\n"
                "The fixture binaries and the generator that produces them are "
                "both committed under tests/fixtures/benchmark/. Neither is "
                "present, which means this is not a complete checkout of the "
                "repository.\n"
                "The --test modes that do not need the fixture are "
                "endpoint-catalog and selected-contract."
            )
        # The binaries are committed; regenerating is a recovery path for a
        # checkout that dropped them (a .gitignore accident, an archive export),
        # not a routine build step. It costs milliseconds and needs no toolchain.
        print("Benchmark fixture binaries missing; regenerating from the generator.")
        subprocess.run(
        ...
        if not benchmark_dll.is_file() or not benchmark_debug_exe.is_file():
            raise RuntimeError(
                f"{generator} ran without producing {benchmark_dll} and "
                f"{benchmark_debug_exe}."
            )
    for program_path in (
...
def run_debugger_live_test(repo_root: Path, mcp_url: str) -> None:
    ...
    if not benchmark_debug_exe.is_file():
        raise DebuggerLiveTestSkipped(
            ...
            "It is committed; regenerate it with "
            "`python tests/fixtures/benchmark/make_fixture.py`."
        )
...
def _benchmark_regression_dir(repo_root: Path) -> Path:
    return repo_root / BENCHMARK_FIXTURE_DIR / "regression"
...
def run_benchmark_yaml_regression(repo_root: Path, mcp_url: str) -> None:
    ...
    Reads every tests/fixtures/benchmark/regression/*.yaml file and verifies its
    ...
    yaml_files = sorted(p for p in reg_dir.glob("*.yaml")) if reg_dir.is_dir() else []
    if not yaml_files:
        # This used to print "skipping" and return success. The baselines are
        # committed now, so their absence is a broken checkout -- and a release
        # gate that reports a pass when its only value-checking step did not run
        # is exactly the failure mode this whole tier exists to prevent.
        raise RuntimeError(
            f"No benchmark regression baselines found in {reg_dir}.\n"
            "tests/fixtures/benchmark/regression/*.yaml is committed; if it is "
            "missing the checkout is incomplete. The YAML regression is the "
            "only part of the release tier that checks an answer rather than "
            "checking that an endpoint answered, so it must not be skipped "
            "silently."
        )
...
def run_release_regression_tests(repo_root: Path, mcp_url: str) -> None:
    ...
    try:
        ...
        debugger_outcome = f"SKIPPED ({skip})"
    # Name the debugger outcome in the pass line. A tier that prints an
    # unqualified "passed" while one of its six steps quietly skipped is how a
    # gate comes to look like coverage it does not have.
    print(f"Release regression tier passed. Debugger live test: {debugger_outcome}.")
